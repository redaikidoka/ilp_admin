...
import { VwStudents } from '../../models/VwStudents';

// Making Sure EventSource Type is available to avoid compilation issues.
declare var EventSource: any;

/**
 * Api services for the `VwStudents` model.
 */
@Injectable()
export class VwStudentsApi extends BaseLoopBackApi {
  ...
  public create(data: any = undefined) {
    ...
    return result.map((instance: VwStudents) => new VwStudents(instance));
  }
  ...
  public find(filter: LoopBackFilter = undefined) {
    ...
    if (filter) urlParams.filter = filter;
    let result = this.request(method, url, routeParams, urlParams, postBody);
    return result.map((instances: Array<VwStudents>) =>
        instances.map((instance: VwStudents) => new VwStudents(instance))
    );
  }
