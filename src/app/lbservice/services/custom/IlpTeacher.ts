...
import { IlpTeacher } from '../../models/IlpTeacher';

// Making Sure EventSource Type is available to avoid compilation issues.
declare var EventSource: any;

/**
 * Api services for the `IlpTeacher` model.
 */
@Injectable()
export class IlpTeacherApi extends BaseLoopBackApi {
  ...
  public create(data: any = undefined) {
    ...
    return result.map((instance: IlpTeacher) => new IlpTeacher(instance));
  }
  ...
  public find(filter: LoopBackFilter = undefined) {
    ...
    if (filter) urlParams.filter = filter;
    let result = this.request(method, url, routeParams, urlParams, postBody);
    return result.map((instances: Array<IlpTeacher>) =>
        instances.map((instance: IlpTeacher) => new IlpTeacher(instance))
    );
  }
