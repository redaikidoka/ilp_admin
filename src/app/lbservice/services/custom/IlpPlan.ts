...
import { IlpPlan } from '../../models/IlpPlan';

// Making Sure EventSource Type is available to avoid compilation issues.
declare var EventSource: any;

/**
 * Api services for the `IlpPlan` model.
 */
@Injectable()
export class IlpPlanApi extends BaseLoopBackApi {
  ...
  public create(data: any = undefined) {
    ...
    return result.map((instance: IlpPlan) => new IlpPlan(instance));
  }
  ...
  public find(filter: LoopBackFilter = undefined) {
    ...
    if (filter) urlParams.filter = filter;
    let result = this.request(method, url, routeParams, urlParams, postBody);
    return result.map((instances: Array<IlpPlan>) =>
        instances.map((instance: IlpPlan) => new IlpPlan(instance))
    );
  }
