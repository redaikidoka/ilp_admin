...
import { IlpClass } from '../../models/IlpClass';

// Making Sure EventSource Type is available to avoid compilation issues.
declare var EventSource: any;

/**
 * Api services for the `IlpClass` model.
 */
@Injectable()
export class IlpClassApi extends BaseLoopBackApi {
  ...
  public create(data: any = undefined) {
    ...
    return result.map((instance: IlpClass) => new IlpClass(instance));
  }
  ...
  public find(filter: LoopBackFilter = undefined) {
    ...
    if (filter) urlParams.filter = filter;
    let result = this.request(method, url, routeParams, urlParams, postBody);
    return result.map((instances: Array<IlpClass>) =>
        instances.map((instance: IlpClass) => new IlpClass(instance))
    );
  }
