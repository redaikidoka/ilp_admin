...
import { IlsUserLog } from '../../models/IlsUserLog';

// Making Sure EventSource Type is available to avoid compilation issues.
declare var EventSource: any;

/**
 * Api services for the `IlsUserLog` model.
 */
@Injectable()
export class IlsUserLogApi extends BaseLoopBackApi {
  ...
  public create(data: any = undefined) {
    ...
    return result.map((instance: IlsUserLog) => new IlsUserLog(instance));
  }
  ...
  public find(filter: LoopBackFilter = undefined) {
    ...
    if (filter) urlParams.filter = filter;
    let result = this.request(method, url, routeParams, urlParams, postBody);
    return result.map((instances: Array<IlsUserLog>) =>
        instances.map((instance: IlsUserLog) => new IlsUserLog(instance))
    );
  }
